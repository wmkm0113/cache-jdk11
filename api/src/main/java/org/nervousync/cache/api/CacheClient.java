...
public interface CacheClient {
	 ...
	 * <span class="zh-CN">默认步进值</span>
	 */
	long DEFAULT_STEP_VALUE = 1;
	 ...
	 * <span class="zh-CN">操作结果</span>
	 */
	default long decr(@Nonnull final String key) {
		return this.decr(key, DEFAULT_STEP_VALUE);
	}
	...
	/**
	 * <h3 class="en-US">Read the cache value from the cache key which the cache key was given and set expire time by default value</h3>
	 * <h3 class="zh-CN">读取指定缓存键值对应的缓存数据并设置过期时间为默认值</h3>
	 *
	 * @param key <span class="en-US">Cache key</span>
	 *            <span class="zh-CN">缓存键值</span>
	 * @return <span class="en-US">Cache value or null if cache key did not exist, or it was expired</span>
	 ...
	 * <span class="zh-CN">操作结果</span>
	 */
	default long incr(@Nonnull final String key) {
		return this.incr(key, DEFAULT_STEP_VALUE);
	}
